...
import { Image, Eye, X, Camera, ExternalLink, Heart, MessageCircle, ChevronLeft, ChevronRight, Sparkles, Layers, Video } from 'lucide-react';
...
export default function GallerySection({ onOpenBooking, setCursorHover, language }) {
  ...
  useEffect(() => {
    fetch('https://feeds.behold.so/eEBBeqqjBcTrYjz4wrVS')
      ...
      .catch(err => {
        console.warn('Gallery Instagram feed notice:', err);
        setLoading(false);
      });
  }, []);
  ...
  return (
    <section id="gallery-section" className="py-24 bg-[#08080c]/80 backdrop-blur-xs relative overflow-hidden border-t border-white/5 text-left">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#ff5500]/10 border border-[#ff5500]/30 text-[#ff5500] text-xs font-semibold uppercase tracking-widest font-mono">
            ...
            <span>INSTAGRAM LIVE PORTFOLIO</span>
          </div>
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black uppercase tracking-tight text-white font-heading leading-tight overflow-visible">
            GALERÍA DE <span className="text-orange-gradient font-serif-title font-normal italic">TRABAJOS</span>
          </h2>
          <p className="text-slate-300 text-base font-light max-w-2xl mx-auto">
            {isEs 
              ? 'Piezas reales de autor realizadas por nuestro equipo en Santa Cruz, Tabaiba y el TattooTruck, sincronizadas en directo desde nuestro Instagram oficial.'
              : 'Real bespoke ink masterpieces crafted by our artists in Santa Cruz, Tabaiba, and TattooTruck.'}
          </p>
          ...
          {profile && (
            <div className="pt-2 flex items-center justify-center">
              <a
                ...
                className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-gradient-to-r from-purple-900/60 via-pink-900/40 to-amber-900/40 border border-pink-500/30 text-white text-xs font-mono font-bold hover:border-pink-400 transition-all shadow-[0_0_20px_rgba(219,39,119,0.2)]"
              >
                {profile.profilePictureUrl && (
                  <img src={profile.profilePictureUrl} alt="Malibu Tattoo" className="w-5 h-5 rounded-full object-cover" />
                )}
                <span>@{profile.username}</span>
                <span className="text-slate-400 text-[10px]">({(profile.followersCount / 1000).toFixed(1)}k seguidores)</span>
        ...
        <div className="space-y-6 mb-12">
          ...
          <div className="flex items-center justify-center gap-3">
            <div className="inline-flex p-1 rounded-full bg-black/80 border border-white/10 backdrop-blur-md">
              <button
                ...
                onMouseLeave={() => setCursorHover(false)}
                className={`px-5 py-2 rounded-full text-xs font-mono font-extrabold uppercase tracking-wider transition-all cursor-pointer flex items-center gap-2 ${
                  viewMode === 'all_photos'
          ...
          <div className="flex flex-wrap items-center justify-center gap-2.5">
            {[
              { id: 'all', label: isEs ? 'Todos los Estilos' : 'All Styles', icon: Layers },
              ...
              {filteredItems.map((item, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    onMouseEnter={() => setCursorHover(true, 'VER PIEZA')}
                    onMouseLeave={() => setCursorHover(false)}
                    className="group relative rounded-3xl overflow-hidden glass-panel border border-white/10 hover:border-[#ff5500]/50 transition-all duration-500 cursor-pointer text-left flex flex-col justify-between shadow-2xl"
                  >
                    {/* Image visual wrapper */}
                    <div className="relative h-96 overflow-hidden">
                      <img
                        src={item.imageUrl}
                        alt={item.caption || 'Malibu Tattoo Artwork'}
                        loading="lazy"
                      ...
                      <div className="absolute top-4 left-4 flex items-center gap-1.5">
                        <span className="text-[9px] font-mono font-black uppercase tracking-widest px-3 py-1 rounded-full bg-[#ff5500] text-black shadow-lg">
                          {item.mediaType === 'VIDEO' ? 'REEL' : item.photoNumber ? `FOTO ${item.photoNumber}/${item.totalPhotos}` : item.isAlbum ? `ÁLBUM (${item.children?.length || 4} FOTOS)` : 'PIEZA EXCLUSIVA'}
                        </span>
                      </div>
                    ...
                    <div className="p-4 bg-black/60 backdrop-blur-md flex items-center justify-between border-t border-white/10">
                      ...
                      <span className="text-[10px] font-extrabold uppercase tracking-wider text-[#ff5500] group-hover:underline flex items-center gap-1 font-mono">
        ...
        {/* LIGHTBOX MODAL (Full details + multi-photo carousel support) */}
        <AnimatePresence>
          {selectedPost && (
            <motion.div
              initial={{ opacity: 0 }}
            ...
            >
              ...
              >
                ...
                {/* Visual Section (Photo / Carousel Slide) */}
                <div className="lg:col-span-7 relative bg-black flex items-center justify-center min-h-[350px] lg:min-h-[550px] overflow-hidden">
                  ...
                  {/* Carousel Controls (if post has multiple children) */}
                ...
                <div className="lg:col-span-5 p-6 sm:p-8 flex flex-col justify-between space-y-6 overflow-y-auto">
                  ...
                  <div>
                    {/* Top row with tags and close button */}
